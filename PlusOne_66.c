/* 66
You are given a large integer represented as an integer array digits, where each digits[i] is the i^th digit of the integer. 
The digits are ordered from most significant to least significant in left-to-right order. The large integer does not contain any leading 0's.

Increment the large integer by one and return the resulting array of digits.

Example 1:

    Input: digits = [1,2,3]
    Output: [1,2,4]
    Explanation: The array represents the integer 123.
    Incrementing by one gives 123 + 1 = 124.
    Thus, the result should be [1,2,4].

Example 2:

    Input: digits = [4,3,2,1]
    Output: [4,3,2,2]
    Explanation: The array represents the integer 4321.
    Incrementing by one gives 4321 + 1 = 4322.
    Thus, the result should be [4,3,2,2].

Example 3:

    Input: digits = [9]
    Output: [1,0]
    Explanation: The array represents the integer 9.
    Incrementing by one gives 9 + 1 = 10.
    Thus, the result should be [1,0].

Constraints:

    1 <= digits.length <= 100
    0 <= digits[i] <= 9
    digits does not contain any leading 0's.
 
*/

# include <stdio.h>
# include <stdlib.h>

// Note: The returned array must be malloced, assume caller calls free().
int* plusOne(int* digits, int digitsSize, int* returnSize){
    int *result = (int*)malloc(digitsSize * sizeof(int));

    for(int i = 0; i < digitsSize; i++){ result[i] = digits[i]; }
    for(int i = digitsSize - 1; i >= 0; i--){
        if(result[i] < 9){
            result[i] = result[i] + 1;
            *returnSize = digitsSize;
            return result;
        }
        result[i] = 0;
    }
    free(result);

    result = (int*)malloc((digitsSize + 1) * sizeof(int));
    result[0] = 1;

    for(int i = 1; i <= digitsSize; i++){ result[i] = 0; }

    *returnSize = digitsSize + 1;
    return result;
}

int main(){
    int digits[] = {9, 8, 7, 6, 5, 4, 3, 2, 1, 0};
    int digitsSize = sizeof(digits) / sizeof(digits[0]);
    int returnSize = 0;

    int *new_arr = plusOne(digits, digitsSize, &returnSize);
    
    printf("\nreturnSize = %d\n", returnSize);
    for(int i = 0; i < returnSize; i++){ printf("%d", new_arr[i]); }
    free(new_arr);

    return 0;
}

// 0 ms of runtime.
// 10.07 MB of memory.

// --- WORKS BUT HAVE MEMORY OVERFLOW ---
/* 
int* plusOne(int* digits, int digitsSize, int* returnSize){
    int num = 0;
    for(int i = 0; i < digitsSize; i++){ num = num * 10 + digits[i]; }
    num += 1;

    int length = 0, temp = num;
     
    while(temp > 0){ length++; temp /= 10; }

    int *result = (int*)malloc(length * sizeof(int));

    for(int i = length - 1; i >= 0; i--){
        result[i] = num % 10;
        num /= 10;
    }
    *returnSize = length;

    return result;
}
*/
